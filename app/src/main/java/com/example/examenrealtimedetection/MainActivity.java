package com.example.examenrealtimedetection;
...
import com.example.examenrealtimedetection.ml.CreateEdificioModel;
...
public class MainActivity extends AppCompatActivity implements OnSuccessListener<Text>,
    ...
    public void buscarEdificio (View view){
        txtResults.setText("Nombre del edificio: " + nombreEdificio);
    }
    ...
    public void PersonalizedModel(View v) {
        try {
            CreateEdificioModel model = CreateEdificioModel.newInstance(getApplicationContext());
            TensorImage image = TensorImage.fromBitmap(mSelectedImage);

            CreateEdificioModel.Outputs outputs = model.process(image);
            ...
            nombreEdificio = probability.get(0).getLabel();
            txtResults.setText(probability.get(0).getLabel());
            model.close();
        } catch (IOException e) {
